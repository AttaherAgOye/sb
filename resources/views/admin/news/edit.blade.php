...
@section('header', 'Modifier l\'Actualité')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="{{ route('news.show', $news->slug) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="category" name="category" required 
                        ...
                        <option value="services" {{ old('category', $news->category) == 'services' ? 'selected' : '' }}>Services</option>
                        <option value="partenariats" {{ old('category', $news->category) == 'partenariats' ? 'selected' : '' }}>Partenariats</option>
                        <option value="innovations" {{ old('category', $news->category) == 'innovations' ? 'selected' : '' }}>Innovations</option>
                ...
                <div>
                    ...
                    <input type="text" id="author" name="author" value="{{ old('author', $news->author) }}" required 
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea id="excerpt" name="excerpt" rows="3" required 
                              ...
                              placeholder="Résumé court de l'actualité (max 500 caractères)">{{ old('excerpt', $news->excerpt) }}</textarea>
                    @error('excerpt')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    @if($news->image)
                        <div class="mb-4">
                            ...
                            <img src="{{ $news->image_url }}" alt="{{ $news->title }}" class="max-w-xs rounded-lg shadow">
                        </div>
                    @endif
                        ...
                        <div class="space-y-1 text-center">
                            ...
                            <div class="flex text-sm text-gray-600">
                                <label for="image" class="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-blue-500">
                                    <span>{{ $news->image ? 'Changer l\'image' : 'Télécharger une image' }}</span>
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" id="tags" name="tags" value="{{ old('tags', $news->tags ? implode(', ', $news->tags) : '') }}" 
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="15" required 
                          ...
                          placeholder="Rédigez le contenu complet de votre actualité...">{{ old('content', $news->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="space-y-2 text-sm text-gray-600">
                        <div class="flex justify-between">
                            <span>Vues :</span>
                            <span class="font-medium">{{ $news->views_count }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Créé le :</span>
